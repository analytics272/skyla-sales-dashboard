...
**Correction (2026-09-09), reverted same day:** BH4's room count was briefly
changed 18 → 24 (`sales_booking` has 24 distinct rooms under BH4, six "3BHK
Apartment" units already enumerated in `lib/reference/roomTypeMapping.ts`'s
BH4 room list, on top of the 18 standard rooms) — but reverted back to 18
after cross-checking against the business's live Looker Studio report,
which reports BH4's "Rooms Available" as 540 = 18×30 for the same window,
and matched the dashboard exactly on every other figure checked (Revenue,
Available, Occ%, ADR, RevPAR for all 5 operating properties). The 3BHK
units are real, revenue-generating rooms — their revenue is already
correctly counted — but the business's own occupancy/capacity convention
doesn't count them toward BH4's room inventory. **BH4's room count below
(18) is correct as originally listed; no change needed.** See the KPI
reference doc's revision history for the full evidence trail.
...
| HTC | Skyla | 34 | Active |
| JHS | Skyla | 33 | Active |
| BH4 | Aptly | 18 (briefly changed to 24, reverted — see note above) | Active |
